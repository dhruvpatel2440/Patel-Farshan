...
/** The customer-facing line for each step an admin can move an order to. */
const STATUS_PUSH: Record<string, { title: string; body: (n: string) => string }> = {
  placed: {
    title: 'Order placed',
  ...
  confirmed: {
    ...
    body: (n) => `Your order #${n} is confirmed and will be prepared fresh.`,
  },
  packed: {
    title: 'Order packed 📦',
    body: (n) => `Your order #${n} is packed and ready to go.`,
  },
  out_for_delivery: {
    title: 'Out for delivery 🚚',
    body: (n) => `Your order #${n} is out for delivery and will reach you shortly.`,
  },
  delivered: {
    title: 'Delivered 🎉',
    body: (n) => `Your order #${n} has been delivered. Enjoy your farsan!`,
  },
}

/** Tells the customer their order moved. Unknown statuses send nothing. */
